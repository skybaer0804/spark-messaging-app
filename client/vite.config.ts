...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      VitePWA({
        ...
        workbox: {
          ...
          // 소켓 통신(polling) 및 TWA 인증 파일(.well-known)을 서비스 워커 캐싱에서 명시적으로 제외
        navigateFallbackDenylist: [/^\/socket.io/, /^\/.well-known/],
          // 캐싱할 파일 패턴
    ...
    // Koyeb 환경변수(SPARK_SOCKET_URL 등)를 VITE_ 변수로 매핑
    define: {
      'import.meta.env.VITE_SPARK_SOCKET_URL': JSON.stringify(env.SPARK_SOCKET_URL || env.VITE_SPARK_SOCKET_URL),
      'import.meta.env.VITE_SPARK_PROJECT_KEY': JSON.stringify(env.SPARK_PROJECT_KEY || env.VITE_SPARK_PROJECT_KEY),
    },
