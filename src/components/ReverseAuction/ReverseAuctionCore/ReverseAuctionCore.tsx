                                    ...
                                    {roomList.map((room) => (
                                        <Card key={room.roomId}>
                                            <CardHeader>
                                                ...
                                                <Typography variant="h3" style={{ marginTop: '8px' }}>{room.title}</Typography>
                                            </CardHeader>
                                            <CardBody>
            ...
            <Box padding="md" style={{ borderBottom: '1px solid var(--color-border-default)' }}>
                <Stack direction="row" align="center" justify="space-between">
                    ...
                    {userRole === 'demander' && pendingRequests.length > 0 && (
                        <Paper elevation={2} padding="sm" style={{ position: 'absolute', top: '60px', right: '20px', zIndex: 100, width: '300px' }}>
                            <Stack spacing="sm">
                                ...
                                {pendingRequests.map((req) => (
                                    <Paper key={req.socketId} variant="outlined" padding="sm">
                                        <Stack spacing="xs">
                                            <Typography variant="body-small">{req.name} ({req.socketId.substring(0, 6)})</Typography>
                                            <Stack direction="row" spacing="sm">
                                                <Button size="sm" onClick={() => handleApproveRequest(req.socketId)} fullWidth>Accept</Button>
                                                <Button size="sm" variant="secondary" onClick={() => handleRejectRequest(req.socketId)} fullWidth>Reject</Button>
                                            </Stack>
                                        </Stack>
                                    </Paper>
                                ))}
