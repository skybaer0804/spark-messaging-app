...
export class ReverseAuctionStore {
    ...
    public readonly selectedCategory: Signal<Category> = signal('인테리어');
    ...
    private setupEventListeners() {
        ...
        this.participantService.onRoomMessage({
            ...
            onUserJoined: (participant) => {
                ...
                const isApprovedSupplier = this.participantService.getPendingRequests().find((r) => r.socketId === participant.socketId) === undefined;
        ...
        if (this.chatService) {
            const chatUnsubscribe = sparkMessagingClient.onRoomMessage((msg: RoomMessageData) => {
                // 현재 Room의 메시지만 처리
                if (msg.room !== this.roomService?.getCurrentRoomRef()) {
                    return;
                }
            ...
            // cleanup에 추가하기 위해 저장
            (this as any).chatUnsubscribe = chatUnsubscribe;
        }
    ...
    public async leaveRoom(): Promise<void> {
        ...
        try {
            ...
            const status = this.connectionService?.getConnectionStatus();
            if (status?.socketId) {
                await this.participantService.sendUserLeft(this.currentRoom.value.roomId, status.socketId, Math.max(0, currentParticipants - 1));
            }
    ...
    // 정리
    public cleanup(): void {
        if ((this as any).chatUnsubscribe) {
            (this as any).chatUnsubscribe();
        }
